...
void registration();
void lobby();
int gmailValidation(char email[50]);
...
void login(){
    ...
    int a = gmailValidation(lEmail);

    if(a == 0){
        printf("Invalid gmail.Please enter gmail correctly.\n");
        login();
    }
...
void registration(){
    ...
    scanf("%s",userEmail);
    int c = gmailValidation(userEmail);
    if(c == 0){
        printf("Invalid gmail.Please enter gmail correctly.\n");
        registration();
    }
    ...
    if(eFound == -1){
        for(int gcc=0; gcc<50; gcc++){
            info[globalIndex].email[gcc] = userEmail[gcc];
        }
        printf("U can register enter your password:");
        scanf("%s",&info[globalIndex].password);
        printf("Enter your name:");
        scanf("%s",&info[globalIndex].name);
        printf("Enter your age:");
        scanf("%d",&info[globalIndex].age);
        info[globalIndex].id = info[globalIndex-1].id+1;
        globalIndex++;
        printingAllData();
        login();
    }else{
...
int gmailValidation(char email[50]){
    ...
    int c=0;
    char check[9];
    char gmail[10] = {'@','g','m','a','i','l','.','c','o','m'};

    int size = sizeof (email)/sizeof(email[0]);
